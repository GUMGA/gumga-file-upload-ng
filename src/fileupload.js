...
function FileUpload($parse, GumgaMimeTypeService, $http, $timeout) {
  ...
  function link($scope, $element, $attrs) {
    ...
    let eventHandlers = {
      ...
      , onProgress: (e, i) => $timeout(() => $scope.queue[i].percent = Math.round((e.loaded / e.total) * 100))
    ...
    $scope.upload = function () {
      ...
      angular.forEach($scope.queue, (curr, i) => {
        ...
        $http({
          ...
          url: endpoint,
          uploadEventHandlers: {
            progress : (e) => eventHandlers.onProgress(e, i),
            loadend : (e) => eventHandlers.onLoadEnd(e, i),
            abort : (e) => eventHandlers.onAbort(e, i),
            error : (e) => eventHandlers.onError(e, i)
          },
          headers: {
...
const module = angular.module('gumga.fileupload', []).directive('gumgaFileUpload', FileUpload);
